...
\begin{document}
	...
	\begin{titlepage}
		...
		{\large Credits gehen raus an: \\ Gabriel Schneider \& Luka Radenovi\'{c} \& Patrick Böpple}\par
		\vfill
		{\today}
	\end{titlepage}
	...
	\begin{definitionbox}[Convex sets \& functions]
		\begin{itemize}
			...
			\[
			\{ z \mid z = \lambda x + (1-\lambda) y, \, \lambda \in [0,1]\}\subseteq C
			\]
			\begin{center}
	...
	\begin{lemmabox}[Second order sufficient conditions for local minimizer]
	...
	\begin{itemize}
		\item Free problems $\to$ only endpoints are constrained.
	...
	also called ``stationary solutions'' of the corresponding CV problem. Why?
	\[
	...
	\begin{lemmabox}[First-order sufficient condition for global minimizers]
		...
		\textcolor{orange}{Every global minimizer is a strong local minimizer.}
	\end{lemmabox}
	...
	\begin{lemmabox}[First order necessary conditions for local minimizers of $(CV-P2)$]
		...
		\begin{enumerate}
			...
			\begin{itemize}
				\item[A)] $[l_{\dot{x}} + \varphi_x] \big|_{x=x^\star, s=s_2^\star} = 0 \quad$ \textcolor{blue}{$\leftarrow$ use if $x(s_2)$ free}
				\item[B)] $[l - \dot{x}^T l_{\dot{x}} + \varphi_s] \big|_{x=x^\star, s=s_2^\star} = 0 \quad$\textcolor{blue}{$\leftarrow$ use if $s_2$ free}
			\end{itemize}
		\end{enumerate}
	\end{lemmabox}
	...
	\begin{lemmabox}[First-Order Necessary Conditions for unconstrained OC]
		...
		\hspace*{1cm}$\left.\begin{array}{l}
			...
			\dot{\lambda}^\star=l_x(t,x^\star,u^\star)-f_x(t,x^\star,u^\star)\lambda^\star,\quad\underset{\substack{\text{\textcolor{blue}{$\uparrow$}} \\ \text{\textcolor{blue}{not if $x(t_f)$ given}}}}{\lambda^\star(t_f)=0}\\
	...
	\begin{lemmabox}[First order sufficient conditions for unconstrained OC - Mangasarian conditions]
		...
		\begin{itemize}
			...
			\item $\lambda^\star(t)\leq0\quad\forall t\in[t_0,t_f]$. \textcolor{blue}{ $\leftarrow$ if $f$ linear not needed}
		\end{itemize}
	...
	\]
	\[
	\mu: \mathcal{X} \times [t_0, t_e] \to \mathcal{U} \subseteq \mathbb{R}^{n_u}
	\]
	...
	\[
	x \in \mathcal{X} := \{x^{(1)}, x^{(2)}, \dots, x^{(N)}\} \quad (\text{Finite state space})
	\]
	\[
	u \in \mathcal{U} := \{u^{(1)}, \dots, u^{(M)}\} \quad (\text{Finite input space})
	\]
	Example: $\mathcal{X} = \{1, 0, -1\}$; $N=3$.
	...
	\[
	[DP-P2] \quad \min_{u_{[t,t_f]}} \quad \int_{t}^{t_f} l(s,x(s),u(s))\,ds + \varphi(x(t_f))
	\]
	\[
	...
	\[
	V(t,x) := \inf_{u_{[t,t_f]}} \left\{ J(t,x,u) \mid \dot{x}=f(t,x,u), u \in \mathcal{U}, (t_f, x(t_f)) \in \mathcal{S} \right\}
	\]
	...
	\begin{lemmabox}[Regularity properties of $V$]
		...
		\end{itemize}
		Then the value fct $V$ is bounded and uniformly Lipschitz continuous.
	...
	\begin{lemmabox}[Principle of Optimality - cont. time]
		...
		\[
		V(t,x) = \inf_{u_{[t, t+\Delta t]}} \int_{t}^{t+\Delta t} l(s, x(s), u(s)) \, ds + V(t+\Delta t, x(t+\Delta t))
		\]
	...
	\[
	V(t,x) = \inf_{u_{[t, t+\Delta t]}} ( l(t,x,u(t))\Delta t + V(t,x) + V_t(t,x)\Delta t + \langle V_x(t,x), f(t,x,u(t))\Delta t \rangle + o(\Delta t) )
	\]
	...
	\]
	where $\mathcal{H}(t,x,u,\lambda) = -l(t,x,u) + \langle \lambda(t), f(t,x,u) \rangle \big|_{\lambda = - V_x(t,x)} = -V_x(t,x)$
	...
	From this, we can easily obtain the value function
	\[
	V(t,x) = \begin{cases}
		e^{(t_f-t)} x, & x < 0 \\
		e^{-(t_f-t)} x, & x > 0 \\
		0, & x = 0
	\end{cases} \qquad x \in \mathbb{R}^{n_x}, \quad t \in [t_0, t_f] \qquad \circledast
	\]
	...
	\begin{center}
		\begin{tikzpicture}
			% Achsen etwas verlängert für bessere Proportionen
			\draw[->] (-3.5,0) -- (3.5,0) node[right] {$x$};
			\draw[->] (0,-2.5) -- (0,1.5) node[right] {$V(t,x)$};
			
			% V(t,x) Funktion mit sehr deutlichem Knick
			% Linker Ast: steilere Steigung (m = 0.8)
			\draw[blue, very thick] (-2.5,-2.0) -- (0,0);
			% Rechter Ast: flachere Steigung (m = 0.2)
			\draw[blue, very thick] (0,0) -- (2.5,0.5);
			
			% Rotes Steigungsdreieck links (unterhalb der Geraden)
			\draw[red, thick] (-2,-1.6) -- (-1,-1.6) -- (-1,-0.8);
			
			% Rotes Steigungsdreieck rechts (unterhalb der Geraden)
			\draw[red, thick] (1,0.2) -- (2,0.2) -- (2,0.4);
			
			% Beschriftungen
			\node[blue, above left] at (-1.2, -1.2) {$\exp(t_f-t)$};
			\node[blue, right] at (2.2, 0.3) {$\sim \exp(-(t_f-t))$};
			
			% Markierung im Ursprung
			\draw[blue, thick] (0,0) circle (2.5pt);
		\end{tikzpicture}
	\end{center}
	
	This is not a pathological case. Non-differentiability arises often for problems with constraints and/or terminal costs.
	
	Note: $V$ has some regularity property (see Lemma \autoref{lem:lemma4.2}). Specifically, it is Lipschitz continuous.
	...
	\begin{center}
		\begin{tikzpicture}[>=stealth, thick, scale=1.1]
			...
			\begin{scope}
				...
				% Steigungsdreieck 1 (Grün oben)
				\draw[green!60!black, thick] (2.6, 2.6) -- (2.8, 2.6) -- (2.8, 2.8);
				
				% Slope 2 (Purple) - Flatter
				\draw[violet, thick] (0.5, 1.7) -- (3.5, 2.3) node[right, pos=1.0, scale=0.8] {$\zeta \in D^+v(x)$};
				% Steigungsdreieck 2 (Lila oben)
				\draw[violet, thick] (2.7, 2.14) -- (3.1, 2.14) -- (3.1, 2.22);
			...
			\begin{scope}[yshift=-4.5cm]
				...
				% Steigungsdreieck 3 (Grün unten)
				\draw[green!60!black, thick] (2.6, 1.28) -- (3.2, 1.28) -- (3.2, 1.36);
				
				% Slope 2 (Purple) - Steeper
				\draw[violet, thick] (0.8, 0.4) -- (3.2, 2.0) node[right, scale=0.8] {$\zeta \in D^-v(x)$};
				% Steigungsdreieck 4 (Lila unten)
				\draw[violet, thick] (2.6, 1.6) -- (2.9, 1.6) -- (2.9, 1.8);
	...
	In 4.1 we found the ``recursion'' $V_k(x_k) = \min\limits_{u_k} g_k(x_k, u_k) + V_{k+1}(f_k(x_k, u_k))$.
	...
	\begin{definitionbox}[Monotone and contractive operators (or mapping)]
		...
		A mapping $\mathcal{K}: S(Y) \to S(Y)$ (``from functions to functions'') is \underline{monotone} iff:
		\[
		\langle \mathcal{K} p_1 - \mathcal{K} p_2, p_1 - p_2 \rangle \ge 0, \quad \forall p_1, p_2 \in S(Y).
		\]
		The definition depends on the choice of inner product. In our case ($n=1$), this simplifies to:
		\[
			p_2(y) \ge p_1(y) \implies (\mathcal{K}p_2)(y) \ge (\mathcal{K}p_1)(y), \quad \forall y \in Y
		\]
		\begin{center}
			\begin{tikzpicture}[>=stealth, thick, font=\small, scale=0.8]
				...
				\draw[->] (0,-0.5) -- (0,3.5) node[left] {$\mathcal{K}(p)$};
				...
				\draw[dashed, blue] (x1) -- (y1) -- (0, 1.15) node[left, black] {$\mathcal{K}p_1$};
				\draw[dashed, blue] (x2) -- (y2) -- (0, 2.2) node[left, black] {$\mathcal{K}p_2$};
				...
				\begin{scope}[xshift=7cm, yshift=-1cm]
					...
					\draw[violet, thick] (-0.2, 0.8) .. controls (0.5, 1.2) and (1.5, 1.0) .. (2.5, 1.5) .. controls (3.0, 2.0) .. (3.8, 2.8) node[right] {$\mathcal{K}p_1$};
					
					% Kp2 darüber
					\draw[violet, thick] (-0.2, 1.8) .. controls (0.5, 2.2) and (1.5, 2.5) .. (3.8, 4.2) node[right] {$\mathcal{K}p_2$};
		...
		$\mathcal{K}: P(Y) \to P(Y)$ is a \underline{contraction} if $\exists \alpha \in [0, 1)$ s.t.:
		\[
		\| \mathcal{K} p_1 - \mathcal{K} p_2 \|_\infty \le \alpha \| p_1 - p_2 \|_\infty, \quad \forall p_1, p_2 \in P(Y).
		\]
	...
	DP operators $\to T_\mu, T : S(\mathcal{X}) \to S(\mathcal{X})$ ($ s : \mathcal{X} \to \mathbb{R}$, like $J^\star, J_\pi$).
	
	\boxed{T_\mu} For given $\mu \in \mathcal{M}$:
	\[
	(T_\mu J)(x) := g(x, \mu(x)) + \alpha J(f(x, \mu(x))), \quad J \in S(\mathcal{X}), x \in \mathcal{X}
	\]
	...
	\begin{enumerate}[label=\protect\textcircled{\arabic*}]
		...
		\[
		J_\mu(x) = g(x, \mu(x)) + \alpha J_\mu(f(x, \mu(x))), \quad x \in \mathcal{X}
		\]
		\[
		...
		\[
		T_{\mu^\star} J^\star(x) = T J^\star(x), \quad x \in \mathcal{X}.
		\]
	\end{enumerate}
	...
	\begin{theorembox}[Theorem 4.3: Monotonicity \& Contraction of operators in DP]
		...
		\begin{itemize}
			\item $\alpha \in (0, 1)$
			\item bounded cost $|g(x,u)| < \infty, \ \forall x \in \mathcal{X}, \forall u \in \mathcal{U}$
		\end{itemize}
	...
	Given $\hat{J} \in P(\mathcal{X})$ approximation of $J^\star$.
	We can obtain a policy $\hat{\mu}$ by "treating $\hat{J}$ as if it was $J^\star$":
	\[
	\hat{\mu}(x) \in \arg\min_{u \in \mathcal{U}(x)} g(x,u) + \alpha \hat{J}(f(x,u)), \quad x \in \mathcal{X}
	\]
	Equivalently: $T_{\hat{\mu}} \hat{J} = T \hat{J}$.
	
	$\hat{\mu}$ is called \underline{one-step} lookahead policy because it solves an optimal control problem with ``one step'' and terminal cost $\alpha\hat{J}$.
	...
	We are given an approximate $J_m$ (like $\hat{J}$ before) and we compute sequences.
	...
	Given an initial estimate $J_0 \in P(\mathcal{X})$, we apply fixed point iteration to the Bellman operator
	\[
	...
	\begin{lemmabox}[Properties of VI]
		Assume contraction of the DP operators, $J_0 \in \mathcal{P}(X)$. Then
		\begin{itemize}
	...
	\begin{itemize}
		\item If $\mathcal{M}$ is finite (set of policies is finite; \autoref{sec:chapter4.1}, MDP etc.) then $\exists \bar{n}$ s.t. $J_{\mu^{\bar{n}}} = J^\star$.
	...
	\[
	h(z_{k+1}) = h(z_k) + \frac{\partial h}{\partial z}(z_k) (z_{k+1} - z_k) = 0
	\]
	\[
	...
	\begin{center}
		\begin{tikzpicture}[
			% Hier definieren wir die mathematische Funktion und ihre erste Ableitung
			declare function={
				f(\x) = 0.04*pow(\x,3) + 0.1*\x + 0.3;
				df(\x) = 0.12*pow(\x,2) + 0.1;
			},
			>=stealth, scale=1.3
			]
			% Achsen
			\draw[thick, ->] (-0.5,0) -- (5.0,0);
			\draw[thick, ->] (0,-0.5) -- (0,4.0);
			
			% --- MATHEMATISCHE BERECHNUNG DER NEWTON-SCHRITTE ---
			% Schritt 1: Startwert x_i
			\pgfmathsetmacro{\xi}{3.8} 
			\pgfmathsetmacro{\yi}{f(\xi)}
			\pgfmathsetmacro{\mi}{df(\xi)}
			
			% Schritt 2: Berechne x_{i+1}
			\pgfmathsetmacro{\xii}{\xi - \yi/\mi} 
			\pgfmathsetmacro{\yii}{f(\xii)}
			\pgfmathsetmacro{\mii}{df(\xii)}
			
			% Schritt 3: Berechne x_{i+2}
			\pgfmathsetmacro{\xiii}{\xii - \yii/\mii} 
			
			% --- ZEICHNEN ---
			
			% Die Kurve selbst (blau)
			\draw[blue, thick, domain=-0.2:4.2, smooth, samples=100] plot (\x, {f(\x)});
			\node[blue, font=\large] at (3.2, 2.7) {$h$}; 
			
			% Iteration i
			\draw[thick] (\xi, 0) node[below=2pt] {$x_i$} -- (\xi, \yi);
			\filldraw (\xi, 0) circle (1.5pt);
			
			% Echte mathematische Tangente an x_i
			\draw[thick] (\xi+0.3, {\yi + \mi*0.3}) -- ({\xii-0.5}, {\mi*(\xii-0.5 - \xi) + \yi});
			
			% Iteration i+1
			\draw[thick] (\xii, 0) node[below=2pt] {$x_{i+1}$} -- (\xii, \yii);
			\filldraw (\xii, 0) circle (1.5pt);
			
			% Echte mathematische Tangente an x_{i+1}
			\draw[thick] (\xii+0.4, {\yii + \mii*0.4}) -- ({\xiii-0.4}, {\mii*(\xiii-0.4 - \xii) + \yii});
			
			% Markierung für x_{i+2} (nur ein kleiner Strich auf der Achse, wie in der Skizze)
			\draw[thick] (\xiii, -0.1) -- (\xiii, 0.1); 
			
			% Beschreibender Text auf der rechten Seite
			\node[align=left, anchor=west] at (5.0, 2.2) {
				Newton method\\
				works well when:\\[1.5ex]
				$\bullet$ the function is\\
				\hspace{2mm} ``mildly nonlinear''\\[1.5ex]
				$\bullet$ we start close to\\
				\hspace{2mm} the zero of\\
				\hspace{2mm} the function
			};
		\end{tikzpicture}
	\end{center}
	...
	\begin{itemize}
		...
		\item $v_k \in \mathbb{R}^{n_x(d+1)}$ vector of coefficients to be determined.
	\end{itemize}
	...
	\begin{center}
		\begin{tikzpicture}[>=stealth, thick, scale=1.3]
			...
			% --- Beschriftungen v_k (angepasst ohne Pfeile auf die Extremwerte) ---
			
			% v0 (über dem Segment)
			\node[blue, font=\large] at (1, 0.2) {$v_0$};
			
			% v1 (unter dem Segment)
			\node[blue, font=\large] at (3, 0.2) {$v_1$};
			
			% v2 (über dem Segment)
			\node[blue, font=\large] at (5, 0.2) {$v_2$};
	...
	\[
	v_k \in \mathbb{R}^{\color{green!60!black}{2n_x}\color{purple}(d+1)}, \quad s_k \in \mathbb{R}^{2n_x}
	\]
	(\textcolor{green!60!black}{$\left[\begin{smallmatrix} x \\ \lambda \end{smallmatrix}\right]$} \quad \textcolor{purple}{degree polynomial} $\to v = \left[\begin{smallmatrix} v_0 \\ \vdots \\ v_{N-1} \end{smallmatrix}\right] \in \mathbb{R}^{N 2n_x(d+1)}$).
	...
	\begin{array}{c}
		...
		\vdots \\
		\dot{P}_k(t_{z,i}, v_k) - g(v_{k,i}, t_{k,i}) \\
		\vdots \\
	...
	\begin{enumerate}[label=\textcircled{\arabic*}]
		...
		\item Even if it exists, $u(x,\lambda)$ can be discontinuous, e.g. bang-bang.\\ $\to$ This leads to discontinuous dynamics $\dot{y}=g(y)$.
	...
	By using integrators, the OC problem becomes an NLP in the space of parameters parametrizing the input $u$.
	...
	\begin{itemize}
		...
		\item the shape of $u$: $\quad u(t) \approx \sum_{i=1}^N q_i u_i(t)$
	\end{itemize}
	
	General problem statement (common to the 3 methods we present).
	
	\[
	\min_{u(\cdot)\in \hat{\mathcal{C}}[0,T]} \quad \int_{0}^{T} l(x,u) dt + \varphi(x(T))
	\]
	\[
	...
	$J^{MS}, h^{MS}$ conceptually the same as SS.
	\[
	J^{MS}(q,s) = \sum_{k=0}^{N-1} l_k(q,s) + \varphi(x(T, q, s))
	\]
	...
	$J^C, h^C$ contains the integrated cost and evaluated constraints at $t_k$ or at collocation times (finer grid).
	
	\# variables:
	\begin{itemize}
		\item $q \in \mathbb{R}^{n_q}$
		\item $\left(s \in \mathbb{R}^{n_x \times N} \quad (\to \text{can be written as a function of } v)\right)$
	...
	\begin{center}
		\begin{tikzpicture}[>=stealth, thick]
			...
			% --- 4. IND Box ---
			\node[draw, rectangle, minimum height=1.5cm, minimum width=2cm, thick] (IND) at (12.5, 1.0) {IND};
			
			% Pfeil nach unten
			\draw[->, thick] (12.5, 0.2) -- (12.5, -0.3);
			
			% --- 5. Indirect Solution Plot (Rechts unten) ---
			\begin{scope}[shift={(11,-3.0)}]
				\node[blue, align=right, above] at (1.5, 2.2) {\small indirect solution};
				\draw[->] (0,0) -- (3,0) node[right] {$t$};
				\draw[->] (0,-0.5) -- (0,2);
				% Glatte Kurve (Violett) - Angepasst an die Treppenfunktion
				\draw[violet, thick, smooth] plot coordinates {(0, 0.8) (0.6, 0.8) (1.0, 0.4) (1.4, 0.4) (1.8, 1.4) (2.2, 1.4) (2.6, 0.6) (2.8, 0.6)};
			...
			% --- 7. Annotation Text ---
			\node[violet, align=center, font=\small] at (7.0, -3.5) {
