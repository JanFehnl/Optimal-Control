...
\begin{document}
	...
	\begin{itemize}
		...
		\[
			J(x):=\int_{s_1}^{s_2}\underset{\rotatebox[origin=c]{180}{$\Lsh$}\text{ running cost, }L:\mathbb{R}\cross\mathbb{R}^n\cross\mathbb{R}^n\to\mathbb{R}}{\underline{l}(s,x(s),\dot{x}(s))ds\hspace*{1.6cm}}
		\]
		\item Bolza form
		\[
			J(x):=\varphi(s_2,x(s_2))+\int_{s_1}^{s_2}l(s,x(s),\dot{x}(s))ds\qquad\varphi:\mathbb{R}\cross\mathbb{R}^n\to\mathbb{R}
		\]
		\item Mayer form
		\[
			J(x):=\varphi(s_2,x(s_2))
		\]
	\end{itemize}
	...
	\begin{itemize}
		...
		\]
		\[
			\Lambda_i(k_i):=\{x\in V\mid\int_{s_1}^{s_2}g_i(s,x(s),\dot{x}(s))ds=k_i\}
		\]
	\end{itemize}
	...
	\begin{definitionbox}[First-variation of a functional]
		...
		\[
			\delta J(x,\xi):=\underset{\eta\to0}{\lim}\frac{J(x+\eta\xi)-J(x)}{\eta}=\left.\frac{\partial J(x+\eta\xi)}{\partial\eta}\right|_{\eta=0}
		\]
		\[
			J:V\to\mathbb{R}
		\]
	...
	\begin{definitionbox}[Second variation of a functional]
		\[
			\delta^2 J(x;\xi):=\left.\frac{\partial^2}{\partial\eta^2}J(x+\eta\xi)\right|_{\eta=0}
		\]
	\end{definitionbox}
