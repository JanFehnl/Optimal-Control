...
\usepackage{tikz}
\usetikzlibrary{arrows.meta, positioning, calc, tikzmark}
\usepackage{pgfplots}
...
\begin{document}
	...
	$\hspace*{4cm}(t_f,x(t_f))\in S\subseteq(t_0,\infty)\cross\mathbb{R}^{n_x}$
	...
	\begin{itemize}
		...
		\item $f:[t_0,t_f]\cross\mathbb{R}^{n_x}\cross\mathbb{R}^{n_u}\to\mathbb{R}^{n_x}$
	...
	\begin{itemize}
		...
		\begin{itemize}
			...
			$$
				S=(\underbrace{t_0,\infty)}_{t_f}\cross\underbrace{\mathbb{R}^{n_x}}_{x(t_f)}
			$$
			\item free-time constrained endpoint case:
			$$
				S=(t_0,\infty)\cross S_f,\qquad S_f\subseteq\mathbb{R}^{n_x}
			$$
			E.g. $S_f=\{x_f\}$
			\begin{center}
				\begin{tikzpicture}
	...
	$$
		\dot{x}=f(t,x,u),\quad x(t_0)=x_0,\quad S=\{t_f\}\cross\mathbb{R}^{n_x}
	$$
	...
	\begin{itemize}
		...
		\item Consider the case where $S=\{t_f\}\cross\underbrace{\{x_f\}}_{\textcircled{1}}\quad\quad\textcircled{1}$ fixed end-point
	...
	\textbf{Note:} If $S=\{t_f\}\cross\{x_f\}$, this Lemma still applies.
	...
	\begin{itemize}
		...
		\item What happens if $S=\{t_f\}\cross\{x_f\}$?
	...
	\begin{align*}
		u\in\hat{\mathcal{C}},\quad&S=\{t_f\}\cross\mathbb{R}^{n_x}\\
		&S=\{t_f\}\cross\{x_f\}
	\end{align*}
	Why?
	\begin{itemize}
