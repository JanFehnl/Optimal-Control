...
\begin{document}
	...
	\begin{lemmabox}[First-Order Necessary Conditions for unconstrained OC]
		...
		\hspace*{1cm}$\left.\begin{array}{cl}
			n_x&\dot{x}^\star=f(t,x^\star,u^\star),\quad x^\star(t_0)=x_0\\
			n_x&\dot{\lambda}^\star=l_x(t,x^\star,u^\star)-f(t,x^\star,u^\star)\lambda^\star,\quad\lambda^\star(t_f)=0\\
			n_u&0=-l_u(t,x^\star,u^\star)+f_u(t,x^\star,u^\star)\lambda^\star,\quad\forall t\in[t_0,t_f]
	...
	\begin{center}
		\begin{tikzpicture}[>=stealth]
			...
			\node[violet, anchor=west] at (-2, 5) {$v \to y$};
	...
	\end{tikzpicture}	
	$$
		+\tikzmarknode{textToUnderlineBlue}{\lambda^T(t_f) y_\eta(t_f; \eta)} - \underbrace{\lambda^T(t_0) y_\eta(t_0; \eta)}_{= 0\text{ because } y(t_0; \eta) = x_0 \; \forall \eta}
	$$
	\begin{tikzpicture}[overlay, remember picture]
	% Stil für die Linien
	\tikzstyle{mystyle} = [line width=1pt, opacity=0.8]
	...
	$$
		...
		[ l_u(t, x, u) - f_u(t, x, u) \lambda ]^T \omega} + \tikzmarknode{textToUnderlineGreen}{[ l_x(t, x, u) - f_x(t, x, u) \lambda - \dot{\lambda} ]^T y_\eta} \, dt
	$$
	\begin{tikzpicture}[overlay, remember picture]
		% Stil für die Linien
		\tikzstyle{mystyle} = [line width=1pt, opacity=0.8]
	...
	\begin{lemmabox}[First-order necessary conditions for opt $\hat{\mathcal{C}}$ control]
		...
		Then $\exists \lambda\in\hat{\mathcal{C}}^1$ such that
		\begin{enumerate}
	...
	B is a generalization of the $2^{nd}$ WE conditions (where $H$ had to be cont.).
	
	Same result holds for $\mathcal{S}=\{t_f\}\times\{x_f\}$.
	
	\subsubsection{Continuous control with general target set}
	
	$$
	\mathcal{S} = \{(t,x) \in (t_0, \infty) \times \mathbb{R}^{n_x} \mid \Psi_k(t,x)=0,\ k=1\dots n_\Psi\}
	$$
	$$
	\to t_f \times x(t_f) \in \mathcal{S} \qquad \Psi = \begin{bmatrix}
	...
	$$
	\mathcal{S} = \{t_f\} \times \mathbb{R}^{n_x} \quad \text{or} \quad \mathcal{S} = \{t_f\} \times \{x_f\}.
	$$
	...
	\begin{itemize}
		...
		\begin{enumerate}
			...
			= \sum \nu_i \nabla \Psi_i
			$$
			If $\Psi = x(t_f) - x_f = 0 \to \nabla_x \Psi$ has full rank. Thus, $\lambda$ is unconstrained $\to$ we retrieve the case we already knew for $\mathcal{S}=\{t_f\}\times\{x_f\}$.
	...
	What if $\mathcal{S} = \{(t,x) \mid \Psi(t,x) \leq 0\}$? What changes?
	\begin{enumerate}
	...
	$$
	$$
	\mathcal{S} = \{t_f\} \times \mathbb{R}^{n_x}
	$$
	$$
	...
	$$
	LQR solution consists of a TPBVP in $(x, \lambda)$ and plugging its solution in $u^\star = R^{-1} B^T \lambda$.
	
	How do we solve TPBVP? Not easy in general. Computational approaches in Chapter 5.
	
	\textbf{Ansatz:} The TPBVP is homogeneous in $x_0$ ($\to$ if $x_0=0$ then $x=\lambda=u\equiv0$ is the solution).
	...
	ODE with final value condition $\xrightarrow{P} Q_f$.
	...
	\subsubsection{Limitations of the variational approach}
	...
	\[
	[OC-P3] \quad \min_{u(\cdot), t_f} \int_{t_0}^{t_f} l(x,u) \, dt
	\]
	\[
	\text{s.t. } \dot{x} = f(x,u), \quad x(t_0) = x_0
	\]
	\[
	...
	\begin{theorembox}[PMP for fixed endpoint]
		...
		\begin{itemize}
			\item Problem $[OC-P3]$
			\item $\mathcal{S} = (t_0, \infty) \times \{x_f\}$
		\end{itemize}
		Suppose $u^\star \in \hat{\mathcal{C}}$ is a strong minimizer with associated final time $t_f^\star$ and $x^\star \in \hat{\mathcal{C}}_n^1$. Then $\exists \lambda \in \hat{\mathcal{C}}^1$ and a constant $\lambda_0^\star \leq 0$ satisfying $(\lambda_0^\star, \lambda^\star(t)) \neq (0,0) \ \forall t \in [t_0, t_f^\star]$ s.t.:
	...
	Second statement: $\mathcal{S}_f = \{x \mid h_i(x)=0, i=1,\dots,n_h\}$.
	...
	\begin{theorembox}[PMP for variable end point]
		...
		\[
		\mathcal{S} = (t_0, \infty) \times \{x \mid h_i(x)=0\} \quad (\text{with regular } x).
		\]
		If ...
		...
		\]
		\[
		\langle \lambda^\star(t_f^\star), d \rangle = 0 \quad \forall d \in \mathcal{T}_{\mathcal{S}_f}(x^\star(t_f^\star)) \quad \Big\} n_x - n_h \text{ conditions}
		\]
		or equivalently $\lambda^\star(t_f^\star) \perp \mathcal{T}_{\mathcal{S}_f}(x^\star(t_f^\star))$
		($\mathcal{T}_{\mathcal{S}_f}$ tangent cone to $\mathcal{S}_f$ at $x^\star(t_f^\star)$, see \autoref{sec:chapter1}).
	...
	\]
	The B.C. prescribes that $\lambda^\star(t_f^\star)$ belongs to the orthogonal complement of $\mathcal{T}_{\mathcal{S}_f}$.
	That is, by definition of $\mathcal{T}_{\mathcal{S}_f}$: $\lambda^\star(t_f^\star)$ a linear combination of $\nabla h_k, \ k=1,\dots,n_h$.
	...
	\begin{aligned}
		...
		&(x, x_{n+1}) \in \mathcal{S}_f \times \{t_f\} && \text{target set for the "state"}
	\end{aligned}
	\right.
	\]
	...
	\[
	\tilde{x} = \begin{bmatrix} x \\ x_{nx+1} \end{bmatrix} \qquad
	\begin{aligned}
		&\dot{x} = f(x_{nx+1},x,u) & x(t_0)=x_0 \\
		&\dot{x}_{nx+1} = 1 & x_{nx+1}(t_0)=t_0 \\
		&l(x_{nx+1}, x, u) & \left\{\begin{aligned}
		&x_{nx+1}(t_f) = t_f \quad \text{if } t_f \text{ fixed} \\
		&x_{nx+1}(t_f) \in \mathbb{R} \quad \text{if } t_f \text{ free}
	\end{aligned}\right.
	\end{aligned}
	\]
	...
	\textcircled{3} $\bar{H} = 0$ because augmented system, the problem is time-invariant.
	
	$\bar{H}:$ Hamiltonian for the augmented system with state $\tilde{x}$.
	\[
	\bar{H} = H + \lambda_{nx+1} \quad (\text{Like in extension \textcircled{1}})
	\]
	$\to H(t,x,u,\lambda_0,\lambda)$ Hamiltonian of the original problem.
	
	Thus: $\dot{\lambda}_{nx+1} = -H_t \neq 0$ because $H$ depends on time.
	
	If $t_f$ is fixed, then we just solve \textcircled{1}+\textcircled{2} and keep in mind that $H$ is now time-varying.
	
	If $t_f$ is free, we need an equation to determine it. We need to obtain a value for $H$ to have a condition \textcircled{3}.
	\[
	\frac{d}{dt} H = H_t, \quad (H(t_f) = -\lambda_{nx+1}(t_f))
	\]
	When $t_f$ is free, what is $\lambda_{nx+1}(t_f)$?
	
	Remember: When $x(t_f)$ is free, $\lambda(t_f)=0$.
	
	Here: $\underbrace{x_{nx+1}(t_f)}_{t_f \text{ free}}$ is free, thus $\lambda_{nx+1}(t_f)=0$.
	
	Therefore we have $\frac{d}{dt}H = H_t$ and $H(t_f) = 0$.
	\[
	H(t) = -\int_{t}^{t_f} H_t(s,x,u,\lambda_0,\lambda)\, ds
	\]
	...
	\begin{enumerate}
		...
		\item $H(t) = -\int_{t}^{t_f} H_t$ B.C. $H(t_f)=0$\quad\textcolor{blue}{$(t_f)$}
	\end{enumerate}
	...
	$t_f, x(t_f)$ are free $\to H = u^T \dots$
	...
	\[
	\bar{H} = \bar{\lambda}_0 \bar{l} + \bar{\lambda}^T f = (\bar{\lambda} + \bar{\lambda}_0 \varphi_x)^T f
	\]
	Both the original ($M$) problem and the reformulated ($L$) problem are time-invariant and $t_f$ free.
	\[
	\to \bar{H} = H = 0
	\]
	So we can relate the variables of the two problems by setting $\bar{H} = H$.
	\[
	...
	\[
		\underset{\text{non-triviality condition would not be satisfied}}{\begin{Bmatrix}
	...
	$\to$ Summary:
	\begin{itemize}
		\item All stays the same except for the B.C. on $\lambda$.
	...
	If ? is satisfied on a non-zero interval $t \in [t_1, t_2]$, then we call this int a \underline{SINGULAR ARC}.
	OC problems with singular arcs are called \underline{SINGULAR OPTIMAL CONTROL PROBLEMS}.
	...
	\[
	\dot{\lambda}^\star = -A^T \lambda^\star \quad (\dot{\lambda} = -H_x)
	\]
	\[
