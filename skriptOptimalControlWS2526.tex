...
\begin{document}
	...
	\begin{lemmabox}[First-order necessary conditions for strong minimizers]
		...
		Then:
		\begin{itemize}
			\item $x^\star$ solves the EE inside the intervals $($\tikzmarknode{meinText1}{$c_i,c_{i+1}$} $),\quad i=0,1,\dots,N\\ c_0=s_1,\; c_{N+1}=s_2$
			
			\begin{tikzpicture}[overlay, remember picture]
				% Stil für die Linien
				\tikzstyle{mystyle} = [line width=1pt, opacity=0.8]
				
				% --- Linien für "meinText1" ---
				\coordinate (A1) at ([yshift=-2pt]meinText1.south west);
				\coordinate (B1) at ([yshift=-2pt]meinText1.south east);
	...
	$x^* \in \hat{\mathcal{C}}^1$ with 1 corner point $c_1 \in (s_1, s_2)$.
	...
	\begin{center}
		\begin{tikzpicture}
			...
			% Label x*(s)
			\node[above right, xshift=10pt, yshift=5pt] at (3.5, 3) {$x^*(s)$};
